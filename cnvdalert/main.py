...
class BugSpider(object):
    ...
    def __init__(self):
        ...
        self.mail = ["mail.haaic.gov.cn", "xxzxaqyw", "xxzx0714"]
    ...
    def getDateUpdate(self, dateResult):
        ...
        yestday = datetime.date.today() - datetime.timedelta(days=4)
    ...
    def sendMail(self, receiveMailName=[], receiveAuthor="" , code=-1):
        nowTime = datetime.datetime.now().strftime("%Y-%m-%d")
        message = MIMEText(u'%s\n%s\n%s' % (u'信息安全漏洞预警:', self.modileText(code), u'******本预警邮件由金盾信安预警系统自动发送，如需退订请回复本邮件******@测试版'), 'plain', 'utf-8')
