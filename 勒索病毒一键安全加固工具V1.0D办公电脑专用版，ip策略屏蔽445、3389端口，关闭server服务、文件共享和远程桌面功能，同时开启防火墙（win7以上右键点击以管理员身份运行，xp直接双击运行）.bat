...
:ICSP
title  金盾勒索病毒一键安全加固工具V1.0D办公电脑专用版
color 0A
cls                 
echo.                      
echo ---------------   金盾勒索病毒一键安全加固工具V1.0D办公电脑专用版  -----------------                                                        
echo                               服务电话9640 ,708室
echo ------------------------------------------------------------------------------------          
...
echo             * 运行脚本后将开启防火墙屏蔽135、137、138、139、445、远程桌面3389端口。
echo             * 关闭server服务将不能使用局域网文件共享、共享打印机。
echo             * 关闭WMI服务可能导致极少部分软件功能不可用，若确实需要再开启。
echo             * 该版本支持xp、win7、win8、win10桌面版系统。
echo             * 如果加固后影响了您的正常工作，可联系我们解决！                                                                                                                            
echo ------------------------------------------------------------------------------------
...
for /f "delims=" %%a in ('ver') do (
	...
	if "!version!" equ "6.1" (
		echo 检测到您是Win7或Win2008操作系统，开始加固.....请等待1分钟......
		echo 如果360等安全软件拦截请允许！
		goto WIN7WIN8WIN10
	) else if "!version!" equ "6.0" (
		echo 检测到您是Vista操作系统，开始加固.....请等待1分钟......
		echo 如果360等安全软件拦截请允许！
		goto WIN7WIN8WIN10
	) else if "!version!" equ "6.2" (
		echo 检测到您是Win8或Win2012操作系统，开始加固.....请等待1分钟......
		echo 如果360等安全软件拦截请允许！
		goto WIN7WIN8WIN10
	) else if "!version!" equ ".0." (
		echo 检测到您是Win10或Win2016操作系统，开始加固.....请等待1分钟......
		echo 如果360等安全软件拦截请允许！
		goto WIN7WIN8WIN10
	) else if "!version!" equ "5.2" (
		echo 检测到您是Win 2003操作系统，开始加固.....请等待1分钟......
		echo 如果360等安全软件拦截请允许！
		goto WIN2003
	) else if "!version!" equ "5.1" (
		echo 检测到您是WinXP操作系统，开始加固.....请等待1分钟......
		echo 如果360等安全软件拦截请允许！
		goto XP
	) else (
...
netsh ipsec static add filter filterlist=Port srcaddr=any dstaddr=Me dstport=3389 protocol=TCP > nul
...
netsh ipsec static add filter filterlist=Port srcaddr=any dstaddr=Me dstport=3389 protocol=UDP > nul
